...
import { blackBright, green, yellow, red } from 'cli-color';
import fs from 'fs';
...
class Logger {
    ...
    debug(message, depth) {
        if (this.debugToFile)
            this._logToFile(LogLevel.DEBUG, message);
        if (this.debugToConsole)
            this._logToConsole(LogLevel.DEBUG, message, this.debugColor, depth);
        return message;
    }
    /**
    ...
    info(message, depth) {
        if (this.infoToFile)
            this._logToFile(LogLevel.INFO, message);
        if (this.infoToConsole)
            this._logToConsole(LogLevel.INFO, message, this.infoColor, depth);
        return message;
    }
    /**
    ...
    warn(message, depth) {
        if (this.warnToFile)
            this._logToFile(LogLevel.WARN, message);
        if (this.warnToConsole)
            this._logToConsole(LogLevel.WARN, message, this.warnColor, depth);
        return message;
    }
    /**
    ...
    error(message, depth) {
        if (this.errorToFile)
            this._logToFile(LogLevel.ERROR, message);
        if (this.errorToConsole)
            this._logToConsole(LogLevel.ERROR, message, this.errorColor, depth);
        return message;
    }
    /**
    ...
    _logToConsole(loglevel, message, color, depth) {
        const toConsole = loglevel === LogLevel.WARN ? console.warn : loglevel === LogLevel.ERROR ? console.error : console.log;
        if (isObject(message)) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            options.instruction =
                (options.instruction ? options.instruction + '\n\n' : '') +
                    `You will be provided with statements, and your task is to convert them to standard ${titleCase(language)}.\n\n` +
...
function funParseFunction(func) {
    var _a, _b, _c, _d, _e;
    const root = (_a = parseScript(func.toString(), {
        tolerant: true,
    }).body) === null || _a === void 0 ? void 0 : _a[0];
    let name = '';
    let params = [];
...
function pdfIteratePages(filepath, begin, end) {
    return __asyncGenerator(this, arguments, function* pdfIteratePages_1() {
        ...
        const doc = yield __await(PDFDocument.load(buffer.buffer));
        if (!end)
        ...
        for (let i = begin; i < end; i++) {
            try {
                const pageDoc = yield __await(PDFDocument.create());
