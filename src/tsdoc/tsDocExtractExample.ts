...
import { regTsDocExampleCode } from '../regexlib/regTsDocExampleCode'
 ...
 * ```
 */
export function tsDocExtractExample(code: string): string {
  const match = rexecFirstMatch(regTsDocExampleCode, code)
  const result = match?.groups?.mid?.replace(/^ \* /gm, '').trim()
  if (!result) throw new Error('Could not find a TSDoc @example in the code.')
  return result
}
