...
describe('getWorkingDirPath', () => {
  it('example', () => {
    expect(/(\\|\/)src$/g.test(util.getWorkingDirPath('src'))).toBe(true);
    expect(
      /(\\|\/)src(\\|\/)libs$/g.test(util.getWorkingDirPath('src', 'libs')),
    ).toBe(true);
  });
});
...
describe('readFileStringSync', () => {
  it('example', () => {
    const sourceThisFile = util.getWorkingDirPath(
      'src',
      'libs',
      'node.test.ts',
    );
    expect(
      util.readFileStringSync(sourceThisFile).indexOf('describe(') !== -1,
    ).toBe(true);
  });
});
...
describe('createFileExtensionFilter', () => {
  ...
  it('creates filter that accepts multiple extensions', () => {
    ...
    expect(paths.filter(filter)).toStrictEqual([
      './src/file.js',
      './src/file.md',
    ]);
  });
...
describe('ensureValidWindowsPath', () => {
  ...
  it('rejects when exceeding max length unless extendedMaxLength is enabled', () => {
    ...
    expect(
      util.ensureValidWindowsPath(longPath, { extendedMaxLength: true }),
    ).toBe(true);
  });
